      ...
      <div style={s.page}>
        <div style={s.topbar}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            ...
            <a href="/admin/reviews" style={s.btnGhost}>⭐ Reviews</a>
        ...
        {previewOpen ? (
          <div style={s.previewOverlay}>
            ...
            <div>
              ...
              <div className="wrap"><EMIInfo content={content} /></div>
        ...
        <Section title="Flash Sale Banner">
          <label style={s.checkboxRow}>
            <input
              type="checkbox"
              checked={!!content.flashSale?.enabled}
              onChange={(e) => update(['flashSale', 'enabled'], e.target.checked)}
            />
            <span>Show flash sale banner</span>
          </label>
          <Field label="Sale title" value={content.flashSale?.title || ''} onChange={(v) => update(['flashSale', 'title'], v)} />
          <Field label="Sale description" value={content.flashSale?.desc || ''} onChange={(v) => update(['flashSale', 'desc'], v)} />
          <Field
            label="Ends at (ISO date-time, e.g. 2026-08-15T23:59:59+05:30)"
            value={content.flashSale?.endsAt || ''}
            onChange={(v) => update(['flashSale', 'endsAt'], v)}
          />
        </Section>
        ...
        <Section title="Mobile Accessories — Product Tabs">
          {content.productCategories.map((cat, ci) => (
            <div key={ci} style={s.card}>
              ...
              {cat.items.map((item, ii) => (
                <div key={ii} style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
                  ...
                  <input
                    ...
                    placeholder="₹149"
                  />
                  <label style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: item.inStock === false ? '#ff5c72' : '#3ddc97', whiteSpace: 'nowrap' }}>
                    <input
                      type="checkbox"
                      checked={item.inStock !== false}
                      onChange={(e) => update(['productCategories', ci, 'items', ii, 'inStock'], e.target.checked)}
                    />
              ...
              <button
                type="button"
                onClick={() => addItem(['productCategories', ci, 'items'], { icon: '📦', name: 'New item', priceFrom: '', inStock: true })}
                style={s.btnAdd}
              >
                + Add item
              </button>
...
const s = {
  ...
  logoBadge: {
    ...
    alignItems: 'center',
    justifyContent: 'center',
    boxShadow: '0 0 22px -4px #159ba0, 0 0 30px -8px #f2a324',
  },
  ...
  btnAdd: {
    ...
    padding: '8px 14px',
    borderRadius: 8,
    border: '1px dashed rgba(21,155,160,.5)',
    background: 'rgba(21,155,160,.08)',
    color: '#5cd6d9',
    fontSize: 12.5,
    fontWeight: 700,
    cursor: 'pointer',
  ...
  btnSave: {
    ...
    borderRadius: 100,
    border: 'none',
    background: 'linear-gradient(120deg,#f2a324,#c9860f)',
    color: '#171200',
    fontSize: 14.5,
    fontWeight: 800,
    cursor: 'pointer',
    boxShadow: '0 10px 24px -8px rgba(242,163,36,.5)',
  },
};
