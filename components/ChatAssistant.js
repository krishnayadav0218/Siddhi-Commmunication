...
function buildReply(query, content) {
  ...
  if (/price|cost|charge|₹|rate/.test(q)) {
    return `Prices vary by item — check the Products section for "From ₹X" pricing, or message us on WhatsApp for an exact quote.`;
  }
  ...
  const matchedFaq = (faqs || []).find((f) =>
    q.split(' ').some((word) => word.length > 3 && f.q.toLowerCase().includes(word))
  );
...
export default function ChatAssistant({ content }) {
  ...
  const [messages, setMessages] = useState([
    { from: 'bot', text: `Hi! 👋 I'm the ${content.brand} assistant. Ask me about hours, location, prices, or repairs.` },
  ]);
  ...
  const suggestions = ['Are you open now?', 'Where are you located?', 'Screen repair price?', 'Do you do AEPS?'];
