...
function getResults(content, need, budgetKey) {
  ...
  if (need.type === 'product') {
    ...
    if (!cat) return { type: 'product', items: [] };
    const maxPrice = budgetKey ? BUDGETS.find((b) => b.key === budgetKey).max : Infinity;
    const items = cat.items
      .filter((it) => it.inStock !== false)
      .filter((it) => parsePriceValue(it.priceFrom) <= maxPrice)
      .map((it) => ({ ...it, id: getProductId(cat.key, it.name), price: parsePriceValue(it.priceFrom) }));
    return { type: 'product', items };
  }
  if (need.type === 'service') {
    return { type: 'service', items: content.services.filter((s) => need.match.some((m) => s.title.includes(m))) };
  }
  if (need.type === 'repair') {
    return { type: 'repair', items: content.repairFeatures.map((f) => ({ icon: '🛠️', title: f.title, desc: f.desc })) };
  }
  return null;
}
...
export default function FinderQuiz({ content }) {
  ...
  return (
    <section id="finder" className="section-pad">
          ...
          <div className="finder-card">
            {!need ? (
              ...
              <>
                ...
                <button className="finder-back" onClick={reset} type="button">
                  ← Start over
                </button>
              </>
            ) : (
                    ...
                    results.items.map((it, i) => (
                      <div className="finder-result-item" key={i}>
                        ...
                        {results.type === 'product' ? (
                          <button type="button" className="btn-add-cart finder-add-btn" onClick={() => addItem(it, 1)}>
                            🛒 Add
                          </button>
                        ) : null}
                      </div>
