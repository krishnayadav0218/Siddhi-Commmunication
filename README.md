...
## Multi-theme picker (latest update)

Added a color-theme switcher in the header (swatch icon, opens a small dropdown) with 3 options — no layout/design changes, only color tokens swap:

- **Vibrant** (default) — the violet/pink/amber Gen-Z palette from the last redesign.
- **Pulse** (new secondary theme) — a cooler teal/cyan-on-deep-navy palette, modeled on a reference design the shop owner provided (`lib/ThemeContext.js` has the exact swatch values, `[data-theme="pulse"]` in `styles/globals.css` has the full token set).
- **Light** — the existing light mode.

Choice persists across visits (`localStorage`). Since every component already reads colors from CSS variables (`--blue`, `--gold`, `--pink`, etc.) rather than hardcoded hex, adding a theme was just a new variable block — no component markup changed.

## Six new features (previous update)
